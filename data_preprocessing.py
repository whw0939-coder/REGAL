...
class DatasetBuilder:
    ...
    def _node_type(self, n):
        lbl = self._node_label(n)
        if lbl:
            return lbl.lower()
        tf = (n.get("typeFullName") or n.get("dynamicTypeHintFullName") or "")
    ...
    def _node_name(self, n):
        for k in ("name", "fullName", "methodFullName", "astParentFullName"):
            v = n.get(k)
            if isinstance(v, str) and v.strip():
                return v
        return ""
    ...
    def _call_name(self, n):
        """
        More reliable call name extraction for CALL nodes:

        - Prioritize methodFullName/name (if it's something like 'ns::foo:int(char*)', take the last identifier part)

        - Return to the (callee(...)) regular expression for code
        """
    ...
    def _node_tokens(self, n):
        if self._is_identifier_node(n):
            return self._identifier_name(n)
        return self._node_code(n)
    ...
    def _has_pointer_type(self, n):
        tf = n.get("typeFullName")
        if isinstance(tf, list):
            tf = tf[0] if tf else ""
        tf = str(tf or "")
        return "*" in tf

    def _is_parameter_node(self, _clip01, n):
    ...
    def compute_avg_out_degree(self, nodes, adjacency_map):
        n = max(1, len(nodes))
        s = sum(len(adjacency_map.get(u, [])) for u in nodes)
        return self._clip01(s / n / 3.0)

    def compute_internal_edge_density(self, nodes, adjacency_map):
        node_set = set(nodes)
        n = max(1, len(nodes))
        edges_in = 0
        for u in nodes:
            for v in adjacency_map.get(u, []):
                if v in node_set:
                    edges_in += 1
        return self._clip01(edges_in / n / 4.0)

    def approx_cyclomatic(self, nodes, adjacency_map):
        node_set = set(nodes)
        n = max(1, len(nodes))
        edges_in = 0
        for u in nodes:
            for v in adjacency_map.get(u, []):
                if v in node_set:
                    edges_in += 1
        cyclo = max(0, edges_in - n + 1)
        return self._log_norm(cyclo, scale=5.0)

    def approx_max_nesting(self, nodes, ast_nodes_dict, adjacency_map, max_depth_cap=10):
        typed = set(["if", "ifstmt", "for", "forstmt", "while", "whilestmt", "switch", "switchstmt", "try", "catch"])
        node_set = set(nodes)

        def is_struct(nid):
            t = self._node_type(ast_nodes_dict[nid])
            return any(tt in t for tt in typed)

        children = {u: [v for v in adjacency_map.get(u, []) if v in node_set] for u in nodes}
        visited = set()
        best = 0
    ...
    def stmt_ratios(self, nodes, ast_nodes_dict):
        n = max(1, len(nodes))
        c = {"call": 0, "if": 0, "loop": 0, "switch": 0, "ret": 0}
        for nid in nodes:
            node = ast_nodes_dict[nid]
            lbl = self._node_label(node)
            if lbl == "CALL":
                c["call"] += 1
            elif lbl == "CONTROL_STRUCTURE":
                ...
                c["ret"] += 1
        return [
            self._clip01(c["call"] / n), self._clip01(c["if"] / n), self._clip01(c["loop"] / n),
            self._clip01(c["switch"] / n), self._clip01(c["ret"] / n)
        ]

    def api_usage_features(self, nodes, ast_nodes_dict):
        DANGEROUS_APIS = {
            "strcpy", "strcat", "sprintf", "vsprintf", "gets", "getwd", "scanf", "sscanf",
            "streadd", "strecpy", "realpath", "system", "popen", "execl", "execle", "execlp", "execv", "execvp",
            "execve",
            "memcpy"
        }
        SAFE_ALT_APIS = {
            "strncpy", "strlcpy", "strncat", "snprintf", "vsnprintf", "fgets", "getline", "memmove"
        }
        ALLOC_APIS = {"malloc", "calloc", "realloc", "new", "fopen", "open", "socket"}
        FREE_APIS = {"free", "delete", "fclose", "close"}

        dang = safe = alloc = free = 0
        fmt_n_percent = 0
        fmt_mismatch = 0
        total_calls = 0
        ...
        for nid in nodes:
            ...
            if not name:
                continue
            if name in DANGEROUS_APIS: dang += 1
            if name in SAFE_ALT_APIS:  safe += 1
            if name in ALLOC_APIS:     alloc += 1
            if name in FREE_APIS:      free += 1
            ...
            if m:
                fmt = m.group(1)
                fmt_items = len(re.findall(r"%[-+0-9#\.\*]*(?:[diuoxXfFeEgGaAcspn])", fmt))
                arg_commas = code.count(",")
                if arg_commas > 0 and abs(fmt_items - arg_commas) >= 2:
                    fmt_mismatch += 1

        total_calls = max(1, total_calls)
        dang_ratio = self._clip01(dang / total_calls)
        safe_ratio = self._clip01(safe / total_calls)
        alloc_free_imbalance = self._clip01(self._safe_div(max(0, alloc - free), max(1, alloc)))
        fmt_n_ratio = self._clip01(fmt_n_percent / total_calls)
        fmt_mismatch_ratio = self._clip01(fmt_mismatch / total_calls)
        return [dang_ratio, safe_ratio, alloc_free_imbalance, fmt_n_ratio, fmt_mismatch_ratio]

    def pointer_cast_features(self, nodes, ast_nodes_dict):
        deref = arrow = cast = 0
        for nid in nodes:
            code = self._node_tokens(ast_nodes_dict[nid])
            deref += code.count("*")
            arrow += code.count("->")
            cast += len(re.findall(r"\([ \t]*[A-Za-z_][A-Za-z0-9_ \t\*]*\)", code))
        n = max(1, len(nodes))
        return [self._clip01(deref / n / 4.0), self._clip01(arrow / n / 2.0), self._clip01(cast / n / 3.0)]

    def identifier_keyword_hits(self, nodes, ast_nodes_dict):
        KEYWORDS = {"buf", "buffer", "len", "length", "size", "cap", "idx", "index", "off", "offset",
                    "src", "dst", "cmd", "path", "env", "arg", "argv", "argc"}
        cnt = 0;
        tot = 0
        for nid in nodes:
            ids = self._node_identifiers(ast_nodes_dict[nid])
            if not ids:
                continue
            tot += len(ids)
            cnt += sum(1 for x in ids if x.lower() in KEYWORDS)
        return self._clip01(self._safe_div(cnt, max(1, tot)))

    def bounds_check_consistency(self, nodes, ast_nodes_dict):
        ok = bad = 0
        for nid in nodes:
            code = self._node_tokens(ast_nodes_dict[nid])
            c = code.replace(" ", "")
            if re.search(r"[a-zA-Z_]\w*<\s*[a-zA-Z_]\w*", c) or "!=NULL" in c or "!==NULL" in c:
                ok += 1
            if re.search(r"\b(memcpy|memmove|strcpy|strcat)\s*\(", c) and not re.search(r"<|<=|size|len|cap|bound", c,
                                                                                        flags=re.I):
                bad += 1
        total = max(1, len(nodes))
        return self._clip01(ok / total), self._clip01(bad / total)

    def build_region_attrs(self, region, ast_nodes_dict, adjacency_map, high_risk_ids):
        nodes = region["nodes"]
        lines = region.get("lines", [])
        n = max(1, len(nodes))
        node_set = set(nodes)

        # A) Statement/Structure Profile
        r_call, r_if, r_loop, r_switch, r_ret = self.stmt_ratios(nodes, ast_nodes_dict)
        avg_branch = self.compute_avg_out_degree(nodes, adjacency_map)
        internal_den = self.compute_internal_edge_density(nodes, adjacency_map)
        cyclo = self.approx_cyclomatic(nodes, adjacency_map)
        max_nest = self.approx_max_nesting(nodes, ast_nodes_dict, adjacency_map, max_depth_cap=10)

        # B) Dangerous API/alloc-free/formatting
        dang_ratio, safe_ratio, alloc_free_imb, fmt_n_ratio, fmt_mismatch_ratio = self.api_usage_features(nodes,
                                                                                                     ast_nodes_dict)

        # C) Pointers/Conversion
        ptr_deref, ptr_arrow, cast_ratio = self.pointer_cast_features(nodes, ast_nodes_dict)

        # D) Name Keyword Hit
        id_keyword_hit = self.identifier_keyword_hits(nodes, ast_nodes_dict)

        # E) Boundary Consistency (Coarse)
        bound_ok, bound_bad = self.bounds_check_consistency(nodes, ast_nodes_dict)

        # F) Existing simple quantities in the prior: size and span & high-risk density (to be added as input features)
        size_score = self._clip01(len(nodes) / 30.0)
        if lines:
            span = max(lines) - min(lines) + 1
            span_score = self._clip01(span / 50.0)
        else:
            span_score = 0.0
        high_risk_cnt = sum(1 for nid in nodes if nid in high_risk_ids)
        risk_density = self._clip01(self._safe_div(high_risk_cnt, n))

        feats = [
            r_call, r_if, r_loop, r_switch, r_ret,  # 5
            avg_branch, internal_den, cyclo, max_nest,  # 4
            dang_ratio, safe_ratio, alloc_free_imb,  # 3
            fmt_n_ratio, fmt_mismatch_ratio,  # 2
            ptr_deref, ptr_arrow, cast_ratio,  # 3
            id_keyword_hit,  # 1
            bound_ok, bound_bad,  # 2
            size_score, span_score, risk_density  # 3
        ]
        names = [
            "ratio_call", "ratio_if", "ratio_loop", "ratio_switch", "ratio_return",
            "avg_out_degree", "internal_edge_density", "cyclomatic", "max_nesting",
            "danger_api_ratio", "safe_api_ratio", "alloc_free_imbalance",
            "fmt_percent_n", "fmt_mismatch",
            "ptr_deref_density", "ptr_arrow_density", "cast_density",
            "identifier_keyword_hit",
            "bounds_ok_ratio", "bounds_bad_ratio",
            "size_score", "span_score", "risk_density"
        ]
    ...
    def process_ast(self, ast_nodes, ast_edges, high_risk_ids, adjacency_map):
        risk_queue = []
        ...
        risk_queue = [
            nid for nid in high_risk_ids
            if nodes[nid].get('code', '') not in self.filter_common_patterns(ast_nodes)
        ]
        ...
        def traverse_high_risk_ids(current_id, current_level, visited):
            if current_id in visited or current_id not in nodes:
                return
            current_node = nodes[current_id]

            if current_id in high_risk_ids:
                risk_score = self.calculate_risk_score(current_node, adjacency_map, nodes)

                if risk_score > 0.4:  # 阈值过滤
        ...
        def traverse(current_id, current_level, visited):
            if current_id in visited or current_id not in nodes:
                return
            current_node = nodes[current_id]
            ...
            if current_node.get('_label') in ['CONTROL_STRUCTURE', 'BLOCK']:
                new_level = current_level + 1
                subtree = collect_subtree(current_id)
                filtered_subtree = [node for node in subtree if node not in visited]
                regions[new_level][current_id] = filtered_subtree
                visited.add(current_id)
                for child in adjacency_map.get(current_id, []):
                    traverse(child, new_level, visited)
            else:
                visited.add(current_id)
                for child in adjacency_map.get(current_id, []):
                    traverse(child, current_level, visited)

        for hr_id in risk_queue:
            traverse_high_risk_ids(hr_id, 0, visited)
        for child_id in adjacency_map.get(root_node['id'], []):
            traverse(child_id, 0, visited)
